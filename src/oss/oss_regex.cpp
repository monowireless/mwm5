...
*/
								...
								/* pre-set bits...   */
static const oss_regex::CHAR bitarr[] = {1,2,4,8,16,32,64,128};
...
inline void
oss_regex::chset(CHAR c)
{
	bittab[(CHAR) ((c) & BLKIND) >> 3] |= bitarr[(c) & BITIND];
}

#define badpat(x)	(char*)(*nfa = END, x)

//#define store(x)	*mp++ = x
inline void
oss_regex::store(CHAR x) {
	if (_mp < nfa + MAXNFA) {
		*_mp++ = x;
	}
}
...
char *
oss_regex::comp(const char *pat)
{
	const char *p;               /* pattern pointer   */
	//CHAR *_mp=nfa;          /* nfa pointer       */
	_mp = nfa;
 ...
 */
//CHAR oss_regex::chrtyp[oss_regex::MAXCHR]
...
const char *
oss_regex::pmatch(const char *lp, CHAR *ap)
{
	int op, c, n;
	const char *e;		/* extra pointer for CLO */
	const char *bp;		/* beginning of subpat.. */
	const char *ep;		/* ending of subpat..	 */
 ...
 */
const static oss_regex::CHAR deftab[16] = {
	0, 0, 0, 0, 0, 0, 0377, 003, 0376, 0377, 0377, 0207,
	0376, 0377, 0377, 007
};

void
oss_regex::modw(const char *s)
{
	int i;
 ...
 */
bool_t
oss_regex::subs(const char *src, char *dst)
{
	char c;
	int  pin;
	const char *bp;

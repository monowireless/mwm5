...
namespace TWEFONT {
	...
	/**********************************************************
	 * createFontShinonome14_mini [chrs = 637]
	 **********************************************************/
	extern const uint16_t font_Shinonome14k_mini_idx[637];
	extern const uint8_t font_Shinonome14k_mini_data[637*14*2];
	...
	const FontDef& createFontShinonome14_mini(uint8_t id, uint8_t line_space, uint8_t char_space, uint32_t opt) {
		...
		if (font != nullptr) {
			font->font_code = id;
			font->font_name = "Shinonome font 14dot (Shinonome14_mini, 637)";

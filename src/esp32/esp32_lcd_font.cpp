...
namespace TWEARD {
	...
	// set drawing area
	inline void setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1, M5Stack& _M5) {
		_M5.Lcd.setWindow(x0, y0, x1, y1);
	}

	inline void startWrite(M5Stack& _M5) {
		_M5.Lcd.startWrite();
	}

	inline void endWrite(M5Stack& _M5) {
		_M5.Lcd.endWrite();
	}

	inline void windowWrite16(uint16_t c, M5Stack& _M5) {
#if defined(ESP32)
		tft_Write_16(c);
	...
	inline int32_t get_color_force(int n_rows
	...
	int16_t drawChar(const TWEFONT::FontDef& font, int32_t x, int32_t y, uint16_t* str, uint32_t color, uint32_t bg, uint8_t opt, M5Stack& _M5) {
	...
	int16_t drawChar(const TWEFONT::FontDef& font, int32_t x, int32_t y, const char* str, uint32_t color, uint32_t bg, uint8_t opt, M5Stack& _M5) {
		TWEUTILS::Unicode_UTF8Converter utf8conv;
		int32_t x_now = x;
